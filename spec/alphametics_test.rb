...
RSpec.describe Alphametics do
  it("puzzle with three letters") do
    ...
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("solution must have unique value for each letter") do
    puzzle = "A == B"
    expected = {}
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("leading zero solution is invalid") do
    puzzle = "ACA + DD == BD"
    expected = {}
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("puzzle with two digits final carry") do
    puzzle = "A + A + A + A + A + A + A + A + A + A + A + B == BCC"
    expected = { "A" => 9, "B" => 1, "C" => 0 }
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("puzzle with four letters") do
    puzzle = "AS + A == MOM"
    expected = { "A" => 9, "M" => 1, "O" => 0, "S" => 2 }
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("puzzle with six letters") do
    puzzle = "NO + NO + TOO == LATE"
    expected = { "A" => 0, "E" => 2, "L" => 1, "N" => 7, "O" => 4, "T" => 9 }
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("puzzle with seven letters") do
    puzzle = "HE + SEES + THE == LIGHT"
    expected = { "E" => 4, "G" => 2, "H" => 5, "I" => 0, "L" => 1, "S" => 9, "T" => 7 }
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("puzzle with eight letters") do
    puzzle = "SEND + MORE == MONEY"
    expected = { "D" => 7, "E" => 5, "M" => 1, "N" => 6, "O" => 0, "R" => 8, "S" => 9, "Y" => 2 }
    expect(Alphametics.solve(puzzle)).to(eq(expected))
  end
  it("puzzle with ten letters") do
